...
// argument.
func Check(output io.Writer, benchmark io.Writer, profile Profile, cartload cartridgeloader.Loader, spec string, uncapped bool, duration string) error {
	var err error
	...
	runner := func() error {
		...
		var measureFrame int
		var measurePulse *time.Ticker

		if benchmark != nil {
			measureTime = time.Now()
			measureFrame = vcs.TV.GetCoords().Frame
			measurePulse = time.NewTicker(1 * time.Second)
		}
